...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	//FAttributeSignature StrenghtDelegate;
	//StrenghtDelegate.BindStatic(UAuraAttributeSet::GetStrengthAttribute);
	//TagsToAttributes.Add(GameplayTags.Attributes_Primary_Strength, StrenghtDelegate);
...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	// Source = causer of the effect , Target = target of the efffect(owner of this AS)
...
void UAuraAttributeSet::HandleIncomingDamage(const FEffectProperties& Props)
{
	...
	if (LocalIncomingDamage > 0.f)
	{
		...
		if (UAuraAbilitySystemLibrary::IsSuccesfulDebuff(Props.EffectContextHandle))
		{
			Debuff(Props);
		}
	}
...
void UAuraAttributeSet::HandleIncomingXP(const FEffectProperties& Props)
{
	...
	if (Props.SourceCharacter->Implements<UPlayerInterface>() && Props.SourceCharacter->Implements<UCombatInterface>())
	{
		...
		if (NumLevelUps > 0)
		{
			...
			/*SetHealth(GetMaxHealth()); // Level almadan once setTomaxhealth oluyor ve maxhealth degistiginde health degismiyor bunun icin PostAttributeChange override edilebilir
			SetMana(GetMaxMana());*/
			bTopOffHealth = true;
			bTopOffMana = true;
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlockedHit, bool bCriticalHit) const
{
	...
	{
		if (AAuraPlayerController* PC = Cast<AAuraPlayerController>(Props.SourceCharacter->Controller)) /*UGameplayStatics::GetPlayerController(Props.SourceCharacter,0) For network issue */
		{
...
void UAuraAttributeSet::SendXPEvent(const FEffectProperties& Props)
{
	if (Props.TargetCharacter->Implements<UCombatInterface>())
	{
		...
		const ECharacterClass CharacterClass = ICombatInterface::Execute_GetCharacterClass(Props.TargetCharacter); // If interface function is BlueprintNativeEvent  use Execute_ function
