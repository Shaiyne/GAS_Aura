...
{
	ClearSlot(&Spec); // old
	Spec.GetDynamicSpecSourceTags().AddTag(Slot); // new
}
...
void UAuraAbilitySystemComponent::UpdateAbilityStatuses(int32 InLevel)
{
	...
	for (const FAuraAbilityInfo& Info : AbilityInfo->AbilityInformation)
	{
		...
		if (InLevel < Info.LevelRequirement) continue;
		...
		if (GetSpecFromAbilityTag(Info.AbilityTag) == nullptr)
		{
			...
			GiveAbility(AbilitySpec);
			MarkAbilitySpecDirty(AbilitySpec); // Mark the spec as dirty to ensure replication
			ClientUpdateAbilityStatuses(Info.AbilityTag, FAuraGameplayTags::Get().Abilities_Status_Eligible, 1);
		}
	}
}

void UAuraAbilitySystemComponent::SR_SpendSpendPoint_Implementation(const FGameplayTag& AbilityTag)
{
...
void UAuraAbilitySystemComponent::SR_EquipAbility_Implementation(const FGameplayTag& AbilityTag, const FGameplayTag& Slot)
{
	if (FGameplayAbilitySpec* AbilitySpec = GetSpecFromAbilityTag(AbilityTag))
	{
		const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
...
bool UAuraAbilitySystemComponent::GetDescriptionsByAbilityTag(const FGameplayTag& AbilityTag, FString& OutDesription, FString& OutNextLevelDescription)
{
	...
	{
		OutDesription = FString();
	}
	else
	{
		OutDesription = UAuraGameplayAbility::GetLockedDescription(AbilityInfo->FindAbilityInfoForTag(AbilityTag).LevelRequirement);
	}
...
void UAuraAbilitySystemComponent::UpgradeAttribute(const FGameplayTag& AttributeTag)
{
	if (GetAvatarActor()->Implements<UPlayerInterface>())
	{
		...
		{
			SR_UpgradeAttribute(AttributeTag);
		}
	}
}

void UAuraAbilitySystemComponent::SR_UpgradeAttribute_Implementation(const FGameplayTag& AttributeTag)
{
