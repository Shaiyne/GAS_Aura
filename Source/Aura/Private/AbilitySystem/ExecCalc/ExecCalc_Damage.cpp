...
{
	RelevantAttributesToCapture.Add(DamageStatics().ArmorDef); // Kind a short of MMC_Health constructor script what we write
...
void UExecCalc_Damage::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag,FGameplayTag>& Pair : GameplayTags.DamageTypesToResistances)
	{
		...
		float DamageTypeValue = Spec.GetSetByCallerMagnitude(Pair.Key, false); //GetSetByCallerMagnitude find one and other will be 0
...
void UExecCalc_Damage::DetermineDebuff(	const FGameplayEffectCustomExecutionParameters& ExecutionParams,const FGameplayEffectSpec& Spec,FAggregatorEvaluateParameters& EvaluationParameters,TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		{
			// Determine if there was a succesful debuff
